...
template <> struct ValueAdaptor<QJSValue> {
    ...
    void remove(const QString &index)
    {
        _value.deleteProperty(index);
    }
...
template <> struct ArrayAdaptor<QJSValue> : public ValueAdaptor<QJSValue> {
    ...
    bool isEmpty() const
    {
        return _value.property(EnginioString::length).toInt() == 0;
    }
...
inline ObjectAdaptor<QJSValue> ValueAdaptor<QJSValue>::toObject() const
{
    return ObjectAdaptor<QJSValue>(_value, _client);
}
inline ArrayAdaptor<QJSValue> ValueAdaptor<QJSValue>::toArray() const
{
    return ArrayAdaptor<QJSValue>(_value, _client);
}
