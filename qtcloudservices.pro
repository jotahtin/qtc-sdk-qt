...
lessThan(QT_MAJOR_VERSION, 5): error("The QtCloudServices library only supports Qt 5.")
load(configure)
load(qt_parts)

QMAKE_DOCS = $$PWD/doc/qtcloudservicesoverview.qdocconf
load(qt_docs)
OTHER_FILES += \
    doc/qtcloudservices_overview.qdoc \
    doc/qtcloudservicesoverview.qdocconf

contains(CONFIG, coverage) {
  gcc*: {
    message("Enabling test coverage instrumentization")
    coverage.commands = lcov -t "QtCloudServices" -o result1.info -c -d . \
